"""
Base settings for the Playto project.
...
SECRET_KEY = env('SECRET_KEY', default='django-insecure-playto-dev-key-change-in-production')
...
# Database — PostgreSQL required, never SQLite
DATABASES = {
    'default': env.db('DATABASE_URL', default='postgres://playto:playto@localhost:5432/playto')
}
# NeonDB (serverless Postgres) does not support CREATE/DROP DATABASE.
# Tell Django to reuse the same database for tests. Always run tests with --keepdb.
...
CELERY_BEAT_SCHEDULE = {
    'retry-stale-payouts': {
        'task': 'payouts.tasks.retry_stale_payouts',
